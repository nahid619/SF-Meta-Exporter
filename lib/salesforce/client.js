...
export class SalesforceClient {
  ...
  /**
   * Batch-insert up to 200 records of the same SObject type using the
   * Composite Sobjects API.
   *
   * Added for the Backup & Restore module (module 7). Mirrors the
   * simple-salesforce bulk_handler behaviour in SFRewind/core/restore_manager.py.
   *
   * @param {string}   objectName  — e.g. 'Account'
   * @param {object[]} records     — plain field→value objects (no `attributes`)
   * @returns {Array<{ success: boolean, id: string|null, errors: object[] }>}
   */
  async batchInsert(objectName, records) {
    if (!records.length) return []

    // The Composite Sobjects endpoint requires the `attributes.type` field
    // on every record — add it here so callers don't have to.
    const withAttrs = records.map(r => ({
      attributes: { type: objectName },
      ...r,
    }))

    const res = await fetch(`${this.baseUrl}/composite/sobjects`, {
      method:  'POST',
      headers: this.headers,
      body:    JSON.stringify({ allOrNone: false, records: withAttrs }),
    })
    ...
    return res.json() // [ { success, id, errors }, … ]
  }
