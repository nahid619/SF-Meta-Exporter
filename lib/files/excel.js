...
/**
 * Convert a 2D array (AOA) to a CSV string.
 * Cells with commas/quotes/newlines are quoted; internal quotes doubled.
 */
...
/**
 * Build a CSV buffer from headers + rows.
 */
export function buildCsvBuffer(headers, rows) {
  const csv = aoaToCsv([headers, ...rows])
  return Buffer.from('\uFEFF' + csv, 'utf8') // UTF-8 BOM for Excel compat
}